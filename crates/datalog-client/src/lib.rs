...
//!
//! **Provenance.** This is a vendored copy. The original lives in the
//! PlayGround monorepo at `axiom-reload/crates/datalog-client`, which is where
//! the Axiom reloader still uses it. gatekeeper was split out into its own
//! repository and the two function crates that speak to `datalog-db` were left
//! pointing at `../../../axiom-reload/...`, a path that only ever resolved
//! inside the monorepo — so a fresh clone of gatekeeper could not build at all.
//! Copying 239 lines with one dependency is the cheap fix; the cost is that the
//! two copies can drift, so if you change the wire protocol, change both.
//!
