        ...
        if !do_not_send_body {
            match transfer_encoding {
                Some(TransferEncoding::Chunked) => {
                    ...
                    // Streaming responses (SSE in particular) require each
                    // completed reader chunk to reach the socket immediately.
                    // Backpressure is still preserved because this write blocks
                    // on the underlying connection. The previous Encoder::new
                    // buffered up to 8 KiB, hiding low-volume events indefinitely.
